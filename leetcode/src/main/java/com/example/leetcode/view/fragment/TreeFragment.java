...
import com.example.leetcode.arithmetic.TreeProblem;
...
public class TreeFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Tree<String> root = new Tree<>("tree");
        ArrayList<String> data = new ArrayList<>();
        int deep = 5;
        double count = Math.pow(2, deep) - 1;
        for (int i = 0; i < count; ++i) {
            data.add("tree" + i);
        }
        TreeProblem.growth(root, data, deep - 1);

        TreeProblem.preOrderTraverse(root);
    }
