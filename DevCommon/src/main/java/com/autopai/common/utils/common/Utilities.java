...
import com.autopai.common.utils.reflect.ReflectUtil;
...
//            return addBorderForIcons(bitmap, iconBitmapSize);
    ...
    public static int getActivityOriConfig(Activity activity){
    ...
    public static boolean isStatusBarVisible(Context context) {
        ...
        if ( (activity.getWindow().getAttributes().flags & WindowManager.LayoutParams.FLAG_FULLSCREEN) == 0 &&
                (decor.getSystemUiVisibility() & (View.SYSTEM_UI_FLAG_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN)) == 0 ) {
            Log.d(TAG, "WWWV status bar is visible");
            return true;
        } else {
            Log.d(TAG, "WWWV status bar is invisible");
            return false;
        }
    }

    public static final TypedArray getWindowStyle(Context context) {
        if(context != null) {
            Class<?> clz = ReflectUtil.getClazz("com.android.internal.R$styleable");
            Field winStyleField = ReflectUtil.getClassFiled(clz, "Window");
            int[] winStyle = (int[])ReflectUtil.getObjectByFiled(null, winStyleField);

            synchronized (context) {
                TypedArray windowStyle = context.obtainStyledAttributes(winStyle);
                return windowStyle;
            }
        }
        return null;
