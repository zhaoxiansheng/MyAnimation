...
public class Rectangle implements Cloneable
{
	...
	public Rectangle(Point p1, Point p2)
	{
		...
		for(int i = 0; i < p1.getDimension(); i ++)
		{
			...
			{
				throw new IllegalArgumentException("坐标点为先左上角后右下角");
			}
		}
		mMinP = (Point) p1.clone();
		mMaxP = (Point) p2.clone();
	...
	/**
	 * 返回Rectangle左上角的Point
	 * @return Point
	 */
	public Point getMinPoint()
	{
		return (Point) mMinP.clone();
	}

	/**
	 * 返回Rectangle右下角的Point
	 * @return Point
	 */
	public Point getMaxPoint()
	{
		return (Point) mMaxP.clone();
	}
	...
	public Rectangle getUnionRectangle(Rectangle rectangle)
	{
		...
		{
			min[i] = Math.min(mMinP.getFloatCoordinate(i), rectangle.mMinP.getFloatCoordinate(i));
			max[i] = Math.max(mMaxP.getFloatCoordinate(i), rectangle.mMaxP.getFloatCoordinate(i));
		}
...
//		ret.hilbertValue = ((hilbertValue >= rectangle.hilbertValue) ? hilbertValue : rectangle.hilbertValue);
	...
	public float getArea()
	{
		...
		for(int i = 0; i < getDimension(); i ++)
		{
			area *= mMaxP.getFloatCoordinate(i) - mMinP.getFloatCoordinate(i);
		}
	...
	{
		Point p1 = (Point) mMinP.clone();
		Point p2 = (Point) mMaxP.clone();
	...
	{
//		if(description == null)
//			return "Rectangle Low:" + mMinP + "\tHigh:" + mMaxP + "\tHilbert value: " + hilbertValue + "\n";
		return "Rectangle Low:" + mMinP + "\tHigh:" + mMaxP  /* + "\tCenter: " + getCenter() + "\n"*/;
//		return "Rectangle Low:" + mMinP + " High:" + mMaxP + "\n" + description ;
	}
	...
	public float intersectingArea(Rectangle rectangle) 
	{
		...
		{
			float l1 = this.mMinP.getFloatCoordinate(i);
			float h1 = this.mMaxP.getFloatCoordinate(i);
			float l2 = rectangle.mMinP.getFloatCoordinate(i);
			float h2 = rectangle.mMaxP.getFloatCoordinate(i);
	...
	public boolean isIntersection(Rectangle rectangle)
	{
		...
		for(int i = 0; i < getDimension(); i ++)
		{
			if(mMinP.getFloatCoordinate(i) > rectangle.mMaxP.getFloatCoordinate(i) ||
					mMaxP.getFloatCoordinate(i) < rectangle.mMinP.getFloatCoordinate(i))
			{
				return false;
			}
		}
	...
	public boolean enclosure(Rectangle rectangle) 
	{
		...
		for(int i = 0; i < getDimension(); i ++)
		{
			if(rectangle.mMinP.getFloatCoordinate(i) < mMinP.getFloatCoordinate(i) ||
					rectangle.mMaxP.getFloatCoordinate(i) > mMaxP.getFloatCoordinate(i))
				return false;
		}
		return true;
	}
	...
	public boolean equals(Object obj) 
	{
		if(obj instanceof Rectangle)
		{
			...
			if(mMinP.equals(rectangle.getMinPoint()) && mMaxP.equals(rectangle.getMaxPoint()))
				return true;
		}
		return false;
	}
	...
	public float getMinDist(Point point) 
	{
		...
		for(int i = 0; i < getDimension(); i ++)
		{
			...
			float l = mMinP.getFloatCoordinate(i);
			float h = mMaxP.getFloatCoordinate(i);
			float r;
	...
	public float getMinMaxDist(Point point)
	{
		...
		for(int k = 0; k < getDimension(); k ++)
		{
			...
			float s = mMinP.getFloatCoordinate(k);
			float t = mMaxP.getFloatCoordinate(k);
			float rm;
			...
			for(int i = 0; i < getDimension(); i ++)
			{
				...
				float s_ = mMinP.getFloatCoordinate(i);
				float t_ = mMaxP.getFloatCoordinate(i);
