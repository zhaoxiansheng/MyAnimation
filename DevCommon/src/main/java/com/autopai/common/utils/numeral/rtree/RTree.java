...
public class RTree {
	 ...
	 * @return 所有遍历的结点集合
	 */
	public List<RTNode> traversePostOrder(RTNode root, List<RTNode> list) {
		if (root == null)
			throw new IllegalArgumentException("Node cannot be null.");

		if(list == null) {
			list = new ArrayList<RTNode>();
		}

		if (!root.isLeaf()) {
			for (int i = 0; i < root.usedSpace; i++) {
				list = traversePostOrder(((RTDirNode) root).getChild(i), list);
				//list.addAll(nodes);
			}
		}
	...
	/**
	 * @param rectangle
	 * @return 返回从root节点开始与给定的rectangle相交的所有叶子结点的枚举
	 */
	public Enumeration<RTNode> intersection(Rectangle rectangle) {
		class IntersectionEnum implements Enumeration<RTNode> {
			...
			public IntersectionEnum(Rectangle rectangle) {
				nodes = intersection(rectangle, file.readNode(0), null);
				if (nodes.isEmpty()) {
					hasNext = false;
				}
			}
	 ...
	 * @param node
	 *            当前结点
	 * @return 返回从给定的node区域开始与给定的rectangle相交的所有叶子结点的集合
	 */
	public List<RTNode> intersection(Rectangle rectangle, RTNode node, List<RTNode> list) {
			...
			// if(! node.isLeaf())
			{
				for (int i = 0; i < node.usedSpace; i++) {
					if (node.datas[i].isIntersection(rectangle)) {
						// 递归调用
						list = intersection(rectangle, ((RTDirNode) node).getChild(i), list);
						//list.addAll(nodes);
					}
				}
			}
	 ...
	 */
	public List<RTNode> intersection_All(Rectangle rectangle, RTNode node, List<RTNode> list) {
		if (rectangle == null || node == null) {
			throw new IllegalArgumentException("Arguments cannot be null.");
		}
		if (rectangle.getDimension() != getDimension()) {
		...
		if (node.getNodeRectangle().isIntersection(rectangle)) {
			...
			if (!node.isLeaf()) {
				for (int i = 0; i < node.usedSpace; i++) {
					if (node.datas[i].isIntersection(rectangle)) {
						// 递归调用
						list = intersection_All(rectangle, ((RTDirNode) node).getChild(i), list);
						//list.addAll(nodes);
					}
				}
			}
	 ...
	 * @param rectangle
	 *            给定的rectangle
	 * @return 从根节点开始返回所有与给定rectangle相交的叶子结点中的rectangle组成的集合,封装成Data集合
	 */
	public List<Data> intersection_Rectangles(Rectangle rectangle, List<Data> list) {
		RTNode node = file.readNode(0); //get root node
		if (rectangle == null || node == null) {
			throw new IllegalArgumentException("Arguments cannot be null.");
		}
		if (rectangle.getDimension() != getDimension()) {
		...
		if (node.getNodeRectangle().isIntersection(rectangle)) {
			if (node.isLeaf())// 新加的
			{
				...
				for (int i = 0; i < node.usedSpace; i++) {
					if (node.datas[i].isIntersection(rectangle)) {
						// 递归调用
						list = intersection_Rectangles(rectangle, ((RTDirNode) node).getChild(i), list);
						//list.addAll(nodes);
					}
				}
			}
	 ...
	 * @param node
	 *            活动结点
	 * @return 从node节点开始返回所有与给定rectangle相交的叶子结点中的rectangle组成的集合,封装成Data集合
	 */
	public List<Data> intersection_Rectangles(Rectangle rectangle, RTNode node, List<Data> list) {
		if (rectangle == null || node == null) {
			throw new IllegalArgumentException("Arguments cannot be null.");
		}
		if (rectangle.getDimension() != getDimension()) {
	 ...
	 */
	private int pruneBranchList(float nearest, BranchList[] branchList,
			int usedSpace) {
		int last = usedSpace;
		int i;
	...
	public List<RTNode> traverseByLevel(RTNode root) {
		...
		List<RTNode> list = traversePostOrder(root, null);
