...
public class MemoryPageFile extends PageFile {
	private static final String TAG = "MemoryPageFile";
	...
	protected RTNode readNode(int page) throws PageFaultError {
		if (page < 0) {
			Log.e(TAG, "Page number cannot be negative.", new IllegalArgumentException("Page number cannot be negative."));
		...
		if (ret == null) {
			Log.e(TAG,"Invalid page number request.", new PageFaultError("Invalid page number request."));
